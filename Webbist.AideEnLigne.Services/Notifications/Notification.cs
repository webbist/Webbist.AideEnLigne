...
namespace Webbist.AideEnLigne.Services.Notifications
{
    ...
    public class Notification(IConfiguration configuration, IUserRepository userRepository)
    {
        ...
        public async Task SendEmailOnUpdateQuestion(Question updatedQuestion)
        {
            ...
            if (updatedQuestion.ModifiedBy != author.Id && (author.NotificationPreference?.NotifyOnMyQuestionOrAnswerModifiedByAdmin ?? false))
            {
                emailTasks.Add(SendEmailAsync(new EmailRequest
                {
                    To = author.Email,
        ...
        public async Task SendEmailOnOfficialAnswer(Answer answer)
        {
            ...
            if (author.NotificationPreference?.NotifyOnAnswerToMyQuestionMarkedOfficial ?? false)
            {
                emailTasks.Add(SendEmailAsync(new EmailRequest
                {
                    To = author.Email,
